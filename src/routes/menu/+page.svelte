<script lang="ts">
	import { selectedQuiz, targetWords, quizMemory } from '$lib/stores/game';

	$: $selectedQuiz;

	function resetQuiz() {
		if ($selectedQuiz) {
			selectedQuiz.set({ ...$selectedQuiz }); // trigger reactivity
			targetWords.set($selectedQuiz.solutions);
			const mem = $quizMemory[$selectedQuiz.id];
			mem.foundWords = [];
		}
		history.back();
	}
...
<div class="menu-wrapper">
	<div class="scroll-area">
		...
		<ul class="space-y-4">
			<li class="flex">
				<a class="menu-btn" href="/puzzles"> 🧩 Puzzles </a>
			</li>
			<li class="menu-btn">⚙️ Settings</li>
			<li class="menu-btn" on:click={shareQuiz}>📤 Share Puzzle</li>
			<li class="menu-btn" on:click={resetQuiz}>🔄 Reset Puzzle</li>
		</ul>
	</div>

	<button class="back-btn mt-4" on:click={() => history.back()}>⬅️ Back</button>
</div>
